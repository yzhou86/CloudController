...
public interface ImageService extends RestService {
	...
	/**
	 * Updates an Image. The image must have the id set or a validation
	 ...
	 *            the image to create
	 * @param payload
	 *            the payload (image data to upload). Note: if the payload is
	 *            null then {@link #reserve(Image)} will be called internally
	 * @return the updated data from the newly stored image
	 */
	Image create(Image image, Payload<?> payload);

	/**
	 * Reserves a new image to be uploaded later. See
	 * {@link #upload(String, Payload, Image)}
	 * 
	 * @param image
	 *            the image to reserve
	 * @return the updated data from the newly stored image
	 */
	Image reserve(Image image);

	/**
	 * Upload image data for a previously-reserved image <br>
	 ...
	 * data associated with it (e.g. not in the queued state), then you will
	 * receive a 409 Conflict exception
	 * 
	 * @param imageId
	 *            the image identifier of the previously reserved image
	 * @param payload
	 *            the playload to upload
	 * @param image
	 *            the optional Image which will be used to update meta data
	...
	/**
	 * List of the other system tenants that may access a given virtual machine
	 ...
	 *            the tenant
	 * @param canShare
	 *            both existing and new memberships will have `can_share` set to
	 *            the provided value
	 * @return true if successful
	 */
	 ...
	 *            the image identifier
	 * @param tenantId
	 *            the tenant to remove (identifier)
	 * @return true if successful
	 */
