...
public class OSClientSession implements OSClient, EndpointTokenProvider {
	...
	public Set<ServiceType> getSupportedServices() {
		if (supports == null)
			supports = Sets.immutableEnumSet(Iterables.transform(
					access.getServiceCatalog(), new ServiceToServiceType()));
		return supports;
	}
		...
		for (Service sc : access.getServiceCatalog()) {
			if (service.getServiceName().equals(sc.getName())) {
				if (sc.getServiceType() == ServiceType.NETWORK) {
					sc.getEndpoints().get(0).toBuilder()
							.type(sc.getServiceType().name());
	...
	private String getEndpointURL(Endpoint endpoint) {
		if (endpoint.getAdminURL() != null) {
			if (getPublicIP() != null
					&& !getPublicIP().equals(endpoint.getAdminURL().getHost())) {
				return endpoint
						.getAdminURL()
						.toString()
