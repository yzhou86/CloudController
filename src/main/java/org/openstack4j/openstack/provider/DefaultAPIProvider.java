...
public class DefaultAPIProvider implements APIProvider {
	...
	private static final Map<Class<?>, Object> instances = Maps
			.newConcurrentMap();
	...
	public void initialize() {
		...
		bind(BlockVolumeSnapshotService.class,
				BlockVolumeSnapshotServiceImpl.class);
		bind(ComputeSecurityGroupService.class,
				ComputeSecurityGroupServiceImpl.class);
	...
	public <T> T get(Class<T> api) {
		...
		if (bindings.containsKey(api)) {
			try {
				...
				e.printStackTrace();
				throw new ApiNotFoundException("API Not found for: "
						+ api.getName(), e);
			}
		}
