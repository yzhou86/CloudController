...
public class NovaKeypair implements Keypair {
	...
	/**
	 * Used internally by the domain side of the API to create a new Keypair on
	 * an OpenStack server
	 * 
	 * @param name
	 *            the name of the keypair
	 * @param publicKey
	 *            the public key or null to have OS generated one
	 * @return NovaKeypair
	 */
	...
	public String toString() {
		return Objects.toStringHelper(this).omitNullValues().add("name", name)
				.add("public_key", publicKey).add("fingerprint", fingerprint)
				.toString();
	}
