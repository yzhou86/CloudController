 ...
 * @author Jeremy Unruh
 */
public class ComputeSecurityGroupServiceImpl extends BaseComputeServices
	...
	public List<? extends SecGroupExtension> list() {
		return get(SecurityGroups.class, uri("/os-security-groups")).execute()
				.getList();
	}
	...
	public List<? extends SecGroupExtension> listServerGroups(String serverId) {
		...
		return get(
				...
				uri("/os-security-groups/servers/​%s/os-security-groups",
						serverId)).execute().getList();
	}
	...
	public SecGroupExtension get(String securityGroupId) {
		...
		return get(NovaSecGroupExtension.class,
				uri("/os-security-groups/%s", securityGroupId)).execute();
	}
	...
	public void delete(String securityGroupId) {
		checkNotNull(securityGroupId);
		delete(Void.class, uri("/os-security-groups/%s", securityGroupId))
				.execute();
	}
	 ...
	 */
	@Override
	public SecGroupExtension update(String securityGroupId, String name,
			String description) {
		checkNotNull(securityGroupId);
		checkNotNull(name);
		return put(NovaSecGroupExtension.class,
				uri("/os-security-groups/%s", securityGroupId)).entity(
				NovaSecGroupExtension.create(name, description)).execute();
	}
	...
	public Rule createRule(Rule rule) {
		checkNotNull(rule);
		return post(SecurityGroupRule.class, uri("/os-security-group-rules"))
				.entity(rule).execute();
	}
	...
	public void deleteRule(String ruleId) {
		checkNotNull(ruleId);
		delete(Void.class, uri("/os-security-group-rules/%s", ruleId))
				.execute();
	}
}
