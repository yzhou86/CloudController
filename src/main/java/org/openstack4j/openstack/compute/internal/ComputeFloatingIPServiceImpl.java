 ...
 */
public class ComputeFloatingIPServiceImpl extends BaseComputeServices implements
	...
	public List<? extends FloatingIP> list() {
		return get(NovaFloatingIPs.class, uri("/os-floating-ips")).execute()
				.getList();
	}
	...
	public List<String> getPoolNames() {
		return get(NovaFloatingIPPools.class, uri("/os-floating-ip-pools"))
				.execute().getList();
	}
	...
	public FloatingIP allocateIP(String pool) {
		...
		return post(NovaFloatingIP.class, uri).json(
				String.format("{ \"pool\": \"%s\" }", pool)).execute();
	}
	 ...
	 */
	@Override
	public ActionResponse addFloatingIP(Server server, String fixedIpAddress,
			String ipAddress) {
		...
		String innerJson = String.format(
				"{ \"fixed_address\":%s, \"address\": \"%s\" }",
				fixedIpAddress, ipAddress);
	...
	private ActionResponse invokeAction(String uri, String action,
			String innerJson) {
		HttpResponse response = post(Void.class, uri).json(
				String.format("{ \"%s\": %s }", action, innerJson))
				.executeWithResponse();
