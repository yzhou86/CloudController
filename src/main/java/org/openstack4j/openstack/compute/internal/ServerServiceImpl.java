 ...
 * @author Jeremy Unruh
 */
public class ServerServiceImpl extends BaseComputeServices implements
		ServerService {
	...
	public List<? extends Server> list(boolean detail) {
		return get(Servers.class, uri("/servers" + ((detail) ? "/detail" : "")))
				.execute().getList();
	}
	...
	public ActionResponse action(String serverId, Action action) {
		...
		switch (action) {
		case PAUSE:
			return invokeAction(serverId, "pause");
		case UNPAUSE:
			return invokeAction(serverId, "unpause");
		case LOCK:
			return invokeAction(serverId, "lock");
		case UNLOCK:
			return invokeAction(serverId, "unlock");
		case START:
			return invokeAction(serverId, "os-start");
		case STOP:
			return invokeAction(serverId, "os-stop");
		case RESUME:
			return invokeAction(serverId, "resume");
		case RESCUE:
			return invokeAction(serverId, "rescue");
		case UNRESCUE:
			return invokeAction(serverId, "unrescue");
		case SHELVE:
			return invokeAction(serverId, "shelve");
		case SHELVE_OFFLOAD:
			return invokeAction(serverId, "shelveOffload");
		case UNSHELVE:
			return invokeAction(serverId, "unshelve");
		default:
			return ActionResponse.actionFailed(String.format(
					"Action %s was not found in the list of invokable actions",
					action));
		}
	}
	...
	public String createSnapshot(String serverId, String snapshotName) {
		...
		String body = String.format("{ \"name\": \"%s\", \"metadata\":{} }",
				snapshotName);
		HttpResponse response = executeActionWithResponse(serverId,
				"createImage", body);
	...
	public ActionResponse resize(String serverId, String flavorId) {
		...
		return invokeAction(serverId, "resize",
				String.format("{ \"flavorRef\": \"%s\" }", flavorId));
	}
	...
	public ActionResponse addSecurityGroup(String serverId, String secGroupName) {
		...
		return invokeAction(serverId, "addSecurityGroup",
				String.format("{ \"name\": \"%s\" }", secGroupName));
	}
	...
	public ActionResponse removeSecurityGroup(String serverId,
		...
		checkNotNull(serverId);
		checkNotNull(secGroupName);
		return invokeAction(serverId, "removeSecurityGroup",
				String.format("{ \"name\": \"%s\" }", secGroupName));
	}
	...
	public String getConsoleOutput(String serverId, int numLines) {
		...
		ConsoleOutput c = post(ConsoleOutput.class,
				uri("/servers/%s/action", serverId)).json(
				ConsoleOutput.getJSONAction(numLines)).execute();
	...
	public VNCConsole getVNCConsole(String serverId, Type type) {
		...
		return post(NovaVNCConsole.class, uri("/servers/%s/action", serverId))
				.json(NovaVNCConsole.getJSONAction(type)).execute();
	}
	...
	public Map<String, ? extends Number> diagnostics(String serverId) {
		return get(HashMap.class, uri("/servers/%s/diagnostics", serverId))
				.execute();
	}
	...
	private ActionResponse invokeAction(String serverId, String action,
			String innerJson) {
		HttpResponse response = executeActionWithResponse(serverId, action,
				innerJson);
		if (response.getStatus() == 409) {
			System.out.println("ERROR: "
					+ response.unwrap().readEntity(String.class));
			return ActionResponse.actionFailed(String.format(
					"Cannot '%s' while instance in in state of %s", action,
					action));
		}
		return ActionResponse.actionSuccess();
	}

	private HttpResponse executeActionWithResponse(String serverId,
			String action, String innerJson) {
		return post(Void.class, uri("/servers/%s/action", serverId)).json(
				String.format("{ \"%s\": %s }", action,
						(innerJson != null) ? innerJson : "null"))
				.executeWithResponse();
	}
