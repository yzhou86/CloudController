...
/**
 * When retrieving a specific image the Glance API returns headers in the
 * response. This function is responsible for parsing the headers into an Image.
 * 
 * @author Jeremy Unruh
 */
public class ImageFromHeadersFunction extends Parser implements
	...
	public Image apply(HttpResponse from) {
		...
		ImageBuilder builder = GlanceImage
				.builder()
				...
				.containerFormat(
						ContainerFormat.value(from.header(CONTAINER_FORMAT
								.asHeader())))
				.diskFormat(
						DiskFormat.value(from.header(DISK_FORMAT.asHeader())));
		...
		for (String k : from.headers().keySet()) {
			if (k.indexOf(property) > -1) {
				...
				properties.put(k.substring(property.length()).toLowerCase(),
						from.headers().get(k));
			}
		}
