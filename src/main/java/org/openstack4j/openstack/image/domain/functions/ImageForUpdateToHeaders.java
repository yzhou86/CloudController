...
/**
 * A function which takes an Image and applies the values to headers. The
 * function only applies values that are supported for Update based calls
 * 
 * @author Jeremy Unruh
 */
public class ImageForUpdateToHeaders implements
	...
	public Map<String, Object> apply(@Nullable Image from) {
		...
		if (from.getContainerFormat() != ContainerFormat.UNRECOGNIZED)
			headers.put(CONTAINER_FORMAT.asHeader(), from.getContainerFormat()
					.value());
		...
		if (from.getProperties() != null) {
			for (String k : from.getProperties().keySet()) {
				headers.put(PROPERTY.asHeader() + "-" + k, from.getProperties()
						.get(k));
			}
		}
		return headers;
	...
	private void addIfNotNull(Map<String, Object> headers, ImageHeader header,
			Object value) {
		if (value == null)
			return;
