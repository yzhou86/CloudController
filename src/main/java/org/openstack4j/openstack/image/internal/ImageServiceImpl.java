...
public class ImageServiceImpl extends BaseImageServices implements ImageService {
	...
	public Image get(String imageId) {
		...
		return head(Image.class, uri("/images/%s", imageId)).execute(
				ImageFromHeadersFunction.instance());
	}
	...
	public Image update(Image image) {
		...
		return put(GlanceImage.class, uri("/images/%s", image.getId()))
				.headers(ImageForUpdateToHeaders.instance().apply(image))
				.execute();
	}
	...
	public InputStream getAsStream(String imageId) {
		checkNotNull(imageId);
		HttpResponse response = get(Void.class, uri("/images/%s", imageId))
				.header(HEADER_ACCEPT, CONTENT_TYPE_OCTECT_STREAM)
				.executeWithResponse();
	...
	public Image create(Image image, Payload<?> payload) {
		...
		return post(GlanceImage.class, uri("/images"))
				.headers(ImageForUpdateToHeaders.instance().apply(image))
				.entity(payload).execute();
	}
	...
	public Image reserve(Image image) {
		checkNotNull(image);
		return post(GlanceImage.class, uri("/images")).headers(
				ImageForUpdateToHeaders.instance().apply(image)).execute();
	}
	 ...
	 */
	@Override
	public Image upload(String imageId, Payload<?> payload,
			@Nullable Image image) {
		checkNotNull(imageId);
		checkNotNull(payload);
		return put(GlanceImage.class, uri("/images/%s", imageId))
				.headers(ImageForUpdateToHeaders.instance().apply(image))
				.entity(payload).execute();
	}
	...
	public List<? extends ImageMember> listMembers(String imageId) {
		checkNotNull(imageId);
		return get(Members.class, uri("/images/%s/members", imageId)).execute()
				.getList();
	}
	...
	public boolean addMember(String imageId, String tenantId) {
		...
		return put(Void.class, uri("/images/%s/members/%s", imageId, tenantId))
				.executeWithResponse().getStatus() == 204;
	}
	...
	public boolean addMember(String imageId, String tenantId, boolean canShare) {
		...
		checkNotNull(tenantId);
		return put(Void.class, uri("/images/%s/members/%s", imageId, tenantId))
				.entity(new GlanceImageMember(null, canShare))
				.executeWithResponse().getStatus() == 204;
	}
	...
	public boolean removeMember(String imageId, String tenantId) {
		...
		return delete(Void.class,
				uri("/images/%s/members/%s", imageId, tenantId))
				.executeWithResponse().getStatus() == 204;
	}
