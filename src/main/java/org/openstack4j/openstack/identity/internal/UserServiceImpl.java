...
public class UserServiceImpl extends BaseOpenStackService implements
		UserService {
	...
	public List<? extends User> listTenantUsers(String tenantId) {
		...
		return get(Users.class, uri("/tenants/%s/users", tenantId)).execute()
				.getList();
	}

	@Override
	public User create(String tenantId, String name, String password,
			String email, boolean enabled) {
		checkNotNull(name);
		checkNotNull(password);
		return create(KeystoneUser.builder().name(name).tenantId(tenantId)
				.email(email).password(password).enabled(enabled).build());
	}
	...
	public User enableUser(String userId, boolean enabled) {
		checkNotNull(userId);
		return put(KeystoneUser.class,
				uri("/users/%s/OS-KSADM/enabled", userId)).entity(
				KeystoneUser.builder().enabled(enabled).build()).execute();
	}

	@Override
	public User update(User user) {
		checkNotNull(user);
		return put(KeystoneUser.class, uri("/users/%s", user.getId())).entity(
				user).execute();
	}
	...
	public void changePassword(String userId, String password) {
		checkNotNull(userId);
		checkNotNull(password);
		put(Void.class, uri("/users/%s/OS-KSADM/password", userId)).entity(
				KeystoneUser.builder().id(userId).password(password).build())
				.execute();
	}
	...
	public List<? extends Role> listRolesOnTenant(String userId, String tenantId) {
		return Apis.getIdentityServices().roles()
				.listRolesForUser(userId, tenantId);
	}
	...
	public List<? extends Role> listRolesOnCurrentTenant(User user) {
		checkNotNull(user);
		return Apis.getIdentityServices().roles()
				.listRolesForUser(user.getId(), user.getTenantId());
	}
}
