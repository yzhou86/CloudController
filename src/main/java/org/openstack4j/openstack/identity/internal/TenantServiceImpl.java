...
public class TenantServiceImpl extends BaseOpenStackService implements
	...
	public Tenant getByName(String tenantName) {
		...
		return get(KeystoneTenant.class, PATH_TENANTS)
				.param("name", tenantName).execute();
	}

	@Override
	public Tenant create(Tenant tenant) {
		checkNotNull(tenant);
		return post(KeystoneTenant.class, PATH_TENANTS).entity(tenant)
				.execute();
	}
	...
	public Tenant update(Tenant tenant) {
		checkNotNull(tenant);
		return post(KeystoneTenant.class, uri("/tenants/%s", tenant.getId()))
				.entity(tenant).execute();
	}
}
