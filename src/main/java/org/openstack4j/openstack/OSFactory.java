...
public class OSFactory {
	...
	/**
	 * Skips Authentication and created the API around a previously cached
	 * Access object. This can be useful in multi-threaded environments or
	 * scenarios where a client should not be re-authenticated due to a token
	 * lasting 24 hours
	 * 
	 * @param access
	 *            an authorized access entity which is to be used to create the
	 *            API
	 * 
	 * @return the OSCLient
	 */
	 ...
	 * 
	 * @param endpoint
	 *            the endpoint URL of the identity service
	 ...
	 *            true if an HTTPS self-signed environment
	 * @return self for method chaining
	 */
	 ...
	 *             if the credentials or default tenant is invalid
	 */
	public OSClient authenticate() throws AuthenticationException {
		...
		HttpRequest<KeystoneAccess> request = HttpRequest
				.builder(KeystoneAccess.class).endpoint(endpoint)
				.method(HttpMethod.POST).path("/tokens").entity(credentials)
				.build();
		KeystoneAccess access = HttpExecutor.create()
				.execute(request, useNonStrictSSL)
				.getEntity(KeystoneAccess.class);
		return OSClientSession.createSession(
				access.applyContext(endpoint, credentials), useNonStrictSSL);
	}
