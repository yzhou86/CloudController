 ...
 * @author Jeremy Unruh
 */
public class MeterServiceImpl extends BaseTelemetryServices implements
		MeterService {
	...
	public List<? extends Meter> list() {
		CeilometerMeter[] meters = get(CeilometerMeter[].class, uri("/meters"))
				.execute();
	...
	public List<? extends Sample> samples(String meterName) {
		...
		CeilometerSample[] samples = get(CeilometerSample[].class,
				uri("/meters/%s", meterName)).execute();
	...
	public List<? extends Statistics> statistics(String meterName, int period) {
		...
		CeilometerStatistics[] stats = get(CeilometerStatistics[].class,
				uri("/meters/%s/statistics", meterName)).param(period > 0,
				"period", period).execute();
