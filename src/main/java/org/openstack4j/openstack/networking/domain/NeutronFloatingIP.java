...
public class NeutronFloatingIP implements NetFloatingIP {
	...
	public String toString() {
		return Objects.toStringHelper(this).omitNullValues().add("id", id)
				.add("routerId", routerId).add("tenantId", tenantId)
				.add("floatingNetworkId", floatingNetworkId)
				.add("floatingIpAddress", floatingIpAddress)
				.add("fixedIpAddress", fixedIpAddress).add("portId", portId)
				.addValue("\n").toString();
	}
	 ...
	 * @author Nathan Anderson
	 */
	public static class FloatingIPConcreteBuilder implements
