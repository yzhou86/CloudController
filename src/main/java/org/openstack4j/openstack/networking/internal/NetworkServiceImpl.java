 ...
 * @author Jeremy Unruh
 */
public class NetworkServiceImpl extends BaseNetworkingServices implements
		NetworkService {
	...
	public Network get(String networkId) {
		checkNotNull(networkId);
		return get(NeutronNetwork.class, uri("/networks/%s", networkId))
				.execute();
	}
	...
	public Network create(Network network) {
		...
		return post(NeutronNetwork.class, uri("/networks")).entity(network)
				.execute();
	}
