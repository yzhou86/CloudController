 ...
 */
public class FloatingIPServiceImpl extends BaseNetworkingServices implements
	...
	public NetFloatingIP get(String id) {
		checkNotNull(id);
		return get(NeutronFloatingIP.class, uri("/floatingips/%s", id))
				.execute();
	}
	...
	public NetFloatingIP create(NetFloatingIP floatingIp) {
		...
		return post(NeutronFloatingIP.class, uri("/floatingips")).entity(
				floatingIp).execute();
	}
	...
	public NetFloatingIP associateToPort(String id, String portId) {
		...
		return put(NeutronFloatingIP.class, uri("/floatingips/%s", id)).json(
				json).execute();
	}
	...
	public NetFloatingIP disassociateFromPort(String id) {
		checkNotNull(id);
		String json = String.format("{ \"%s\": %s }", "floatingip",
				"{ \"port_id\":null }");
		return put(NeutronFloatingIP.class, uri("/floatingips/%s", id)).json(
				json).execute();
	}
