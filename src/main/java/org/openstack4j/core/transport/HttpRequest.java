...
public class HttpRequest<R> {
	...
	public HttpRequest(String endpoint, String path, HttpMethod method,
			ModelEntity entity, Class<R> returnType) {
	...
	public static final class RequestBuilder<R> {
		 ...
		 * @return this
		 */
		public RequestBuilder<R> endpointFunction(
		 ...
		 * @return the request builder
		 */
		public RequestBuilder<R> endpointTokenProvider(
				EndpointTokenProvider provider) {
		...
		public HttpRequest<R> build() {
			if (provider != null) {
				...
				request.useNonStrictSSLClient = provider
						.useNonStrictSSLClient();
				request.getHeaders().put(ClientConstants.HEADER_X_AUTH_TOKEN,
						provider.getTokenId());
			}
			return request;
		}
