 ...
 * responsible for interfacing with Jersey and mapping common status codes,
...
public class HttpExecutor implements HttpExecutorService {
	 ...
	 *             the exception
	 */
	private <R> HttpResponse invoke(HttpRequest<R> request,
			boolean useNonStrictSSL) throws Exception {
		Client client = ClientFactory.create(useNonStrictSSL);
		WebTarget target = client.target(request.getEndpoint()).path(
				request.getPath());
		...
		Entity<?> entity = (request.getEntity() == null) ? null : Entity
				.entity(request.getEntity(), request.getContentType());
		try {
			if (entity != null) {
				return HttpResponse.wrap(invocation.method(request.getMethod()
						.name(), entity));
			}
			if (HttpMethod.PUT == request.getMethod() || request.hasJson()) {
				return HttpResponse.wrap(invocation.method(request.getMethod()
						.name(), Entity.entity(request.getJson(),
						ClientConstants.CONTENT_TYPE_JSON)));
			}
			return HttpResponse.wrap(invocation.method(request.getMethod()
					.name()));
			...
			throw HttpResponse.mapException(e.getResponse().getStatusInfo()
					.toString(), e.getResponse().getStatus(), e);
		}
	}

	private <R> WebTarget populateQueryParams(WebTarget target,
			HttpRequest<R> request) {

		if (!request.hasQueryParams())
			return target;

		for (Map.Entry<String, List<Object>> entry : request.getQueryParams()
				.entrySet()) {
	...
	private <R> void populateHeaders(Invocation.Builder invocation,
			HttpRequest<R> request) {
