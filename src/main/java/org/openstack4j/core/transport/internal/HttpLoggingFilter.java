...
public class HttpLoggingFilter implements ContainerRequestFilter,
		ClientRequestFilter, ContainerResponseFilter, ClientResponseFilter,
		WriterInterceptor {

	private static final Logger LOGGER = Logger
			.getLogger(HttpLoggingFilter.class.getName());
	private static final String NOTIFICATION_PREFIX = "* ";
	private static final String REQUEST_PREFIX = "> ";
	private static final String RESPONSE_PREFIX = "< ";
	private static final String ENTITY_LOGGER_PROPERTY = HttpLoggingFilter.class
	...
	private void printRequestLine(StringBuilder b, long id, String method,
			URI uri) {
	...
	private void printPrefixedHeaders(StringBuilder b, long id,
			final String prefix, MultivaluedMap<String, String> headers) {
	...
	public void filter(ClientRequestContext context) throws IOException {
		...
		// TODO: change to context.getStringHeaders() once the method is added
		// to the API
		printPrefixedHeaders(b, id, REQUEST_PREFIX,
				HeadersFactory.asStringHeaders(context.getHeaders()));

		if (printEntity && context.hasEntity()) {
			OutputStream stream = new LoggingStream(b,
	...
	public void filter(ContainerRequestContext context) throws IOException {
		...
		printRequestLine(b, id, context.getMethod(), context.getUriInfo()
				.getRequestUri());
	...
	public void aroundWriteTo(WriterInterceptorContext writerInterceptorContext)
		...
		LoggingStream stream = (LoggingStream) writerInterceptorContext
	...
	private class LoggingStream extends OutputStream {
		...
		StringBuilder getStringBuilder() {
			...
			entityString = entityString.replaceAll(PASSWORD_PATTERN,
					"\"password\" : \"******\"");
